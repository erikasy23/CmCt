!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!! NAME: icesat_cmctnc_class_mod.f08
!!!
!!! PURPOSE: Defines a class for reading the ICESAT-CMCT files in netCDF
...
MODULE icesat_cmctnc_class_mod

  use icesat_cmct_rec_mod
  use icesat_config_mod
  use netcdf
  ...
  type :: icesat_cmctnc_class
     ...
     character(len=4)      :: campaign    ! ICESat campaign ("L2A", etc.)
     ...
     type( icesat_cmct_rec ) :: inv    ! invalid (fill) values read from file
...
CONTAINS
  ...
  SUBROUTINE init( self, configFile, campaign, status, verbosity )
    ...
    class( icesat_cmctnc_class ), intent(inout) :: self
...
!!!---------------------------------------------------------------
    ...
    print *
    print *, 'In icesat_cmctnc_class_mod%init'
    ...
    if (.not. allocated(self%fileList)) then
       print *, 'icesat_cmctnc_class_mod%init: No file list found.'
    ...
    if (nfStatus /= ICT_NEXT_FILE_OK) then
       print *, 'icesat_cmctnc_class_mod%init: next_file status =', nfStatus
       status = ICT_INIT_NOFILE
       return
    end if
  ...
  FUNCTION next_rec( self, status, invalid, verbosity ) result( retrec )
    ...
    !! INVALID is an icesat_cmct_rec structure filled with the invalid
    !! values (_FillValue) for the variables in the current file.
    type( icesat_cmct_rec ) :: retrec
    integer, intent(out) :: status
    type( icesat_cmct_rec ), intent(out), optional :: invalid
...
!!!---------------------------------------------------------------
    ...
    if ( self%iRec == self%nRecs ) then
       ...
       if (verb >= 1)  print *, "icesat_cmctnc_class_mod%next_rec: ", &
       ...
       case default
          ...
          print '(a,a,i0)', 'icesat_cmctnc_class_mod%next_rec: ', &
    ...
    ncstatus = nf90_get_var( ncid=self%id%ncid, varid=self%id%time, &
         values=retrec%utctime_j2000s, start=iRecArr )
    ...
    ncstatus = nf90_get_var( ncid=self%id%ncid, varid=self%id%wgs, &
         values=retrec%WGS84Elev_m, start=iRecArr )
    ...
    ncstatus = nf90_get_var( ncid=self%id%ncid, varid=self%id%egmmean, &
         values=retrec%EGM08MeanTideElev_m, start=iRecArr )
    ...
    ncstatus = nf90_get_var( ncid=self%id%ncid, varid=self%id%egmtidefree, &
         values=retrec%EGM08TideFreeElev_m, start=iRecArr )
  ...
  SUBROUTINE next_file( self, status, verbosity )
    ...
    do iFile = self%cFileIdx + 1, size(self%fileList)
       ...
       if ( ncstatus == NF90_NOERR ) then
          print '(a,i0,a)',' icesat_cmctnc_class_mod%next_file: Reading file ', &
               iFile, ':'
          print *, trim(full)
          self%cFileIdx = iFile
    ...
    if (status == ICT_NEXT_FILE_END) then
       if (verb >= 1) print '(a,i0)', &
            'icesat_cmctnc_class_mod%next_file: File list exhausted. ifile=', &
            ifile
       return
    end if
    ...
    ncstatus = nf90_inq_varid( ncid=self%id%ncid, &
         name='WGS84ELEV_M', varid=self%id%wgs)
    call check_ncdf( ncstatus, 'getting WGS84ELEV_M varid')
    ncstatus = nf90_get_att( ncid=self%id%ncid, varid=self%id%wgs, &
         name=FILLATTNM, values=self%inv%wgs84elev_m )
    call check_ncdf( ncstatus, 'getting WGS84ELEV_M fill value')

    ncstatus = nf90_inq_varid( ncid=self%id%ncid, &
         name='EGM08MEANTIDEELEV_M', varid=self%id%egmmean)
    call check_ncdf( ncstatus, 'getting EGM08MEANTIDEELEV_M varid')
    ncstatus = nf90_get_att( ncid=self%id%ncid, varid=self%id%egmmean, &
         name=FILLATTNM, values=self%inv%egm08meantideelev_m )
    call check_ncdf( ncstatus, 'getting EGM08MEANTIDEELEV_M fill value')

    ncstatus = nf90_inq_varid( ncid=self%id%ncid, &
         name='EGM08TIDEFREEELEV_M', varid=self%id%egmtidefree)
    call check_ncdf( ncstatus, 'getting EGM08TIDEFREEELEV_M varid')
    ncstatus = nf90_get_att( ncid=self%id%ncid, varid=self%id%egmtidefree, &
         name=FILLATTNM, values=self%inv%egm08tidefreeelev_m )
    call check_ncdf( ncstatus, 'getting EGM08TIDEFREEELEV_M fill value')
  ...
  FUNCTION clean( self, rec, fieldname, verbosity )
    ...
    !! Dummy args
    type( icesat_cmct_rec ), intent(in) :: rec
    ...
    case ( 'wgs84elev_m' )
       elev = rec%wgs84elev_m
       inv  = self%inv%wgs84elev_m
       elevName = 'WGS84Elev_m'

    case ( 'egm08meantideelev_m' )
       elev = rec%egm08meantideelev_m
       inv  = self%inv%egm08meantideelev_m
       elevName = 'EGM08MeanTideElev_m'

    case ( 'egm08tidefreeelev_m' )
       elev = rec%egm08tidefreeelev_m
       inv  = self%inv%egm08tidefreeelev_m
       elevName = 'EGM08TideFreeElev_m'

    case default
       print *, 'icesat_cmctnc_class_mod%clean: Unhandled field requested: ',&
            trim(fieldname)
       print *, 'Stopping.'
       stop 1
    end select
    ...
    if ( elev == inv ) then
       ...
       if (verb >= 1) then
          print *, 'icesat_cmctnc_class_mod%clean: ', &
  ...
  subroutine check_ncdf ( status, msg )
    ...
    if ( status /= nf90_NoErr ) then
       print *
       print *, 'icesat_cmctnc_class_mod: NetCDF error ' // trim(msg)
...
END MODULE icesat_cmctnc_class_mod
