!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!! NAME: icesat_config_mod.f08
!!!
!!! PURPOSE: Module to read the ICESat datasets' JSON configuration files,
!!! which are organized by campaign.  Uses "fson" to parse the file.
!!!
!!! HISTORY: Author: Jeff Guerber, Sigma Space/GSFC 615, 2016-05-20
!!!    Split out of iceat_cism_class_mod.f08 subroutine init.
...
MODULE icesat_config_mod


  implicit none
  private
  public parse_icesat_config
...
CONTAINS
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! parse_icesat_config: Parse the ICESat dataset config files.  Returns
  ...
  SUBROUTINE parse_icesat_config( configfile, top, campaign, verbosity, &
       dir, filelist )
...
!!!---------------------------------------------------------------
    ...
    print *
    print *, 'In icesat_config_mod%parse_icesat_config'
    ...
    if (.not. associated(configp)) then
       print *, 'icesat_config_mod%parse_icesat_config: Could not parse ConfigFile!'
       print *, 'Stopping.'
       stop 1
    end if
    ...
    if (verb >= 1) print *, "parse_icesat_config: ',  &
         'Dir = " // trim(dir)
    ...
    if (.not. associated(fileListJArrp)) then
       print *, 'icesat_config_mod%parse_icesat_config: Could not get file list!'
       print *, 'Stopping.'
       stop 1
    end if
    if (verb >= 2) then
       print *,"parse_icesat_config: fileListJArrp="
    ...
    if (nFiles == 0) then
       ...
       print *, 'parse_icesat_config: No files found!'
       return
    end if

    allocate ( fileList(nFiles) )
    if (verb >= 1) print *,"parse_icesat_config: nFiles=", nFiles
    ...
    do i = 1, nFiles
       ...
       fileList(i) = file
       if (verb >= 1) then
          print '(2a,i0,2a)', "parse_icesat_config: ", &
    ...
    return
  END SUBROUTINE parse_icesat_config


END MODULE icesat_config_mod
