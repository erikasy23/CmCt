...
MODULE datasets_mod
  ...
  use envisat_cmctnc_class_mod
  use envisat_cmct_rec_mod
  use fson
  ...
  type datasets_class
     ...
     character(len=4)  :: envisat_campaign !for mission=envisat, the campaign
     ...
     type( envisat_cmctnc_class ) :: envisat_cmctnc
...
CONTAINS
  ...
  SUBROUTINE open( self, mission, dataset, envisat_campaign, status, verbosity )
    ...
    character(len=*), intent(in) :: dataset
    character(len=*),  intent(in), optional :: envisat_campaign
    ...
    self%envisat_campaign = ''   ! will be reset below if needed


    SEL_MISSION: select case ( trim(self%mission) )

    !! MISSION envisat
    case ( 'envisat' ) SEL_MISSION

       if (present(envisat_campaign)) then
          self%envisat_campaign = envisat_campaign
       else
          print *, 'datasets_mod%open: Campaign needed for mission envisat'
          status = DS_OPEN_ERROR
          return
       end if
       ...
       case ( 'icesat-cism-elev-grn' ) SEL_DS
          call self%icesat_cism%init( configFile=dsCfgFile, &
               campaign=self%envisat_campaign, &
       ...
       !!
       !! DATASETS: envisat-CMCTNC-*-ANT
       !!
       case ( "envisat-cmctnc-wgselev-ant", &
              "envisat-cmctnc-egmmtelev-ant", &
              "envisat-cmctnc-egmtfelev-ant") SEL_DS
          call self%envisat_cmctnc%init( configFile=dsCfgFile, &
               campaign=self%envisat_campaign, &
               status=dsstatus, verbosity=verb )
          if ( dsstatus /= ICT_INIT_OK ) then
             print *, 'datasets_mod%open: Error initializing envisat_cmctnc_class, status=', &
                  dsstatus
             status = DS_OPEN_ERROR
             return
          end if
       ...
       !! blocks.
          !!
          !! This is where I am adding the datasets for Antarctica for the envisat mission
  ...
  SUBROUTINE next_obs ( self, lat, lon, value, status, verbosity )
    ...
    type(envisat_cmct_rec) :: icmct_rec
    ...
    !!
    !! DATASETS: envisat-CMCTNC-*-ANT
    !! From the envisat CMCT Antarctica datasets in netCDF format
    !!
    case ("envisat-cmctnc-wgselev-ant", &
         "envisat-cmctnc-egmmtelev-ant", &
         "envisat-cmctnc-egmtfelev-ant")
       icmct_rec = self%envisat_cmctnc%next_rec( status=nrstatus, verbosity=verb )
       ...
       ! is out of reasonable elevation range.  See envisat_cmctnc%clean for
       ...
       case ( "envisat-cmctnc-wgselev-ant" )
          value = icmct_rec%elev_wgs84_m
          recQual = self%envisat_cmctnc%clean( rec=icmct_rec, &
               fieldname='elev_wgs84_m', verbosity=verb )

       case ( "envisat-cmctnc-egmmtelev-ant" )
          recQual = self%envisat_cmctnc%clean( rec=icmct_rec, &
               fieldname='elev_egm08meantide_m', verbosity=verb )
          value = icmct_rec%elev_egm08meantide_m

       case ( "envisat-cmctnc-egmtfelev-ant" )
          value = icmct_rec%elev_egm08tidefree_m
          recQual = self%envisat_cmctnc%clean( rec=icmct_rec, &
               fieldname='elev_egm08tidefree_m', verbosity=verb )
