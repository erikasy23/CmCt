!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!! NAME: envisat_cmctnc_class_mod.f08
!!!
!!! PURPOSE: Defines a class for reading the envisat-CMCT files in netCDF
...
MODULE envisat_cmctnc_class_mod

  use envisat_cmct_rec_mod
  use envisat_config_mod
  use netcdf
  ...
  type :: envisat_cmctnc_class
     ...
     character(len=4)      :: campaign    ! envisat campaign ("L2A", etc.)
     ...
     type( envisat_cmct_rec ) :: inv    ! invalid (fill) values read from file
...
CONTAINS
  ...
  SUBROUTINE init( self, configFile, campaign, status, verbosity )
    ...
    class( envisat_cmctnc_class ), intent(inout) :: self
...
!!!---------------------------------------------------------------
    ...
    print *
    print *, 'In envisat_cmctnc_class_mod%init'
    ...
    if (nfStatus /= ICT_NEXT_FILE_OK) then
       print *, 'envisat_cmctnc_class_mod%init: next_file status =', nfStatus
       status = ICT_INIT_NOFILE
       return
    end if
  ...
  FUNCTION next_rec( self, status, invalid, verbosity ) result( retrec )
    ...
    !! INVALID is an envisat_cmct_rec structure filled with the invalid
    !! values (_FillValue) for the variables in the current file.
    type( envisat_cmct_rec ) :: retrec
    integer, intent(out) :: status
    type( envisat_cmct_rec ), intent(out), optional :: invalid
...
!!!---------------------------------------------------------------
    ...
    if ( self%iRec == self%nRecs ) then
       ...
       if (verb >= 1)  print *, "envisat_cmctnc_class_mod%next_rec: ", &
       ...
       case default
          ...
          print '(a,a,i0)', 'envisat_cmctnc_class_mod%next_rec: ', &
  ...
  SUBROUTINE next_file( self, status, verbosity )
    ...
    do iFile = self%cFileIdx + 1, size(self%fileList)
       ...
       if ( ncstatus == NF90_NOERR ) then
          print '(a,i0,a)',' envisat_cmctnc_class_mod%next_file: Reading file ', &
               iFile, ':'
          print *, trim(full)
          self%cFileIdx = iFile
    ...
    if (status == ICT_NEXT_FILE_END) then
       if (verb >= 1) print '(a,i0)', &
            'envisat_cmctnc_class_mod%next_file: File list exhausted. ifile=', &
            ifile
       return
    end if
  ...
  FUNCTION clean( self, rec, fieldname, verbosity )
    ...
    !! Dummy args
    type( envisat_cmct_rec ), intent(in) :: rec
    ...
    case default
       print *, 'envisat_cmctnc_class_mod%clean: Unhandled field requested: ',&
            trim(fieldname)
       print *, 'Stopping.'
       stop 1
    end select
    ...
    if ( elev == inv ) then
       ...
       if (verb >= 1) then
          print *, 'envisat_cmctnc_class_mod%clean: ', &
  ...
  subroutine check_ncdf ( status, msg )
    ...
    if ( status /= nf90_NoErr ) then
       print *
       print *, 'envisat_cmctnc_class_mod: NetCDF error ' // trim(msg)
...
END MODULE envisat_cmctnc_class_mod
