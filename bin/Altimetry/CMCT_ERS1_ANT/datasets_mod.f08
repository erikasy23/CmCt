...
MODULE datasets_mod
  ...
  use ers1_cmctnc_class_mod
  use ers1_cmct_rec_mod
  use fson
  ...
  type datasets_class
     ...
     character(len=4)  :: ers1_campaign !for mission=ers1, the campaign
     ...
     type( ers1_cmctnc_class ) :: ers1_cmctnc
...
CONTAINS
  ...
  SUBROUTINE open( self, mission, dataset, ers1_campaign, status, verbosity )
    ...
    character(len=*), intent(in) :: dataset
    character(len=*),  intent(in), optional :: ers1_campaign
    ...
    self%ers1_campaign = ''   ! will be reset below if needed


    SEL_MISSION: select case ( trim(self%mission) )

    !! MISSION ERS1
    case ( 'ers1' ) SEL_MISSION

       if (present(ers1_campaign)) then
          self%ers1_campaign = ers1_campaign
       else
          print *, 'datasets_mod%open: Campaign needed for mission ers1'
          status = DS_OPEN_ERROR
          return
       end if
       ...
       !!
       !! DATASETS: ERS1-CMCTNC-*-ANT
       !!
       case ( "ers1-cmctnc-wgselev-ant", &
              "ers1-cmctnc-egmmtelev-ant", &
              "ers1-cmctnc-egmtfelev-ant") SEL_DS
          call self%ers1_cmctnc%init( configFile=dsCfgFile, &
               campaign=self%ers1_campaign, &
               status=dsstatus, verbosity=verb )
          if ( dsstatus /= ICT_INIT_OK ) then
             print *, 'datasets_mod%open: Error initializing ers1_cmctnc_class, status=', &
                  dsstatus
             status = DS_OPEN_ERROR
             return
          end if
       ...
       !! blocks.
          !!
          !! This is where I am adding the datasets for Antarctica for the ERS1 mission
  ...
  SUBROUTINE next_obs ( self, lat, lon, value, status, verbosity )
    ...
    !!
    !! DATASETS: ERS1-CMCTNC-*-ANT
    !! From the ERS1 CMCT Antarctica datasets in netCDF format
    !!
    case ("ers1-cmctnc-wgselev-ant", &
         "ers1-cmctnc-egmmtelev-ant", &
         "ers1-cmctnc-egmtfelev-ant")
       icmct_rec = self%ers1_cmctnc%next_rec( status=nrstatus, verbosity=verb )
       ...
       ! is out of reasonable elevation range.  See ers1_cmctnc%clean for
