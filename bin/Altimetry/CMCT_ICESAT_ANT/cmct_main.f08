...
program cmct_main
  ...
  print *, 'by Jeff Guerber, Sigma Space Corp/NASA GSFC 615'
  ...
  COMPLOOP:  do comparison = 1, runcfg%nComps
     ...
     associate( modelInfo => runcfg%comps(comparison)%model, &
       ...
       case ( 'antarctica' )
          call cism_ps_init_std_ant( modelInfo%spacing_km, status )
       case default
       ...
       if (status /= 0) then
          print *,'cmct_main: cism_ps_init_std_ant error, stopping, status=', status
          stop 1
       end if
       ...
       call ds%open( mission=obsInfo%mission, dataset=obsInfo%dataset, &
            icesat_campaign=obsInfo%campaign, &
