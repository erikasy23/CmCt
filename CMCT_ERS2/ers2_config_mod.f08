!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!! NAME: ers2_config_mod.f08
!!!
!!! PURPOSE: Module to read the ers2 datasets' JSON configuration files,
!!! which are organized by campaign.  Uses "fson" to parse the file.
!!!
!!! HISTORY: Author: Jeff Guerber, Sigma Space/GSFC 615, 2016-05-20
!!!    Split out of ers2_cism_class_mod.f08 subroutine init.
...
MODULE ers2_config_mod


  implicit none
  private
  public parse_ers2_config
...
CONTAINS
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! parse_ers2_config: Parse the ers2 dataset config files.  Returns
  ...
  SUBROUTINE parse_ers2_config( configfile, top, campaign, verbosity, &
       dir, filelist )
    ...
    character(len=*), intent(in) :: campaign     ! ers2 campaign
...
!!!---------------------------------------------------------------
    ...
    print *
    print *, 'In ers2_config_mod%parse_ers2_config'
    ...
    if (.not. associated(configp)) then
       print *, 'ers2_config_mod%parse_ers2_config: Could not parse ConfigFile!'
       print *, 'Stopping.'
       stop 1
    end if
    ...
    if (verb >= 1) print *, "parse_ers2_config: ',  &
         'Dir = " // trim(dir)
    ...
    if (.not. associated(fileListJArrp)) then
       print *, 'ers2_config_mod%parse_ers2_config: Could not get file list!'
       print *, 'Stopping.'
       stop 1
    end if
    if (verb >= 2) then
       print *,"parse_ers2_config: fileListJArrp="
    ...
    if (nFiles == 0) then
       ...
       print *, 'parse_ers2_config: No files found!'
       return
    end if

    allocate ( fileList(nFiles) )
    if (verb >= 1) print *,"parse_ers2_config: nFiles=", nFiles
    ...
    do i = 1, nFiles
       ...
       fileList(i) = file
       if (verb >= 1) then
          print '(2a,i0,2a)', "parse_ers2_config: ", &
    ...
    return
  END SUBROUTINE parse_ers2_config


END MODULE ers2_config_mod
