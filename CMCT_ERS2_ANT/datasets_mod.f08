...
MODULE datasets_mod
  ...
  type datasets_class
     ...
     character(len=4)  :: ers2_campaign !for mission=ers2, the campaign
     ...
     type( ers2_cmctnc_class ) :: ers2_cmctnc
...
CONTAINS
  ...
  SUBROUTINE open( self, mission, dataset, ers2_campaign, status, verbosity )
    ...
    character(len=*), intent(in) :: dataset
    character(len=*),  intent(in), optional :: ers2_campaign
    ...
    self%ers2_campaign = ''   ! will be reset below if needed


    SEL_MISSION: select case ( trim(self%mission) )

    !! MISSION ers2
    case ( 'ers2' ) SEL_MISSION

       if (present(ers2_campaign)) then
          self%ers2_campaign = ers2_campaign
       else
          print *, 'datasets_mod%open: Campaign needed for mission ers2'
          status = DS_OPEN_ERROR
          return
       end if
       ...
       !!
       !! DATASETS: ers2-CMCTNC-*-ANT
       !!
       case ( "ers2-cmctnc-wgselev-ant", &
              "ers2-cmctnc-egmmtelev-ant", &
              "ers2-cmctnc-egmtfelev-ant") SEL_DS
          call self%ers2_cmctnc%init( configFile=dsCfgFile, &
               campaign=self%ers2_campaign, &
               status=dsstatus, verbosity=verb )
          if ( dsstatus /= ICT_INIT_OK ) then
             print *, 'datasets_mod%open: Error initializing ers2_cmctnc_class, status=', &
                  dsstatus
             status = DS_OPEN_ERROR
             return
          end if
       ...
       !! blocks.
          !!
          !! This is where I am adding the datasets for Antarctica for the ers2 mission
  ...
  SUBROUTINE next_obs ( self, lat, lon, value, status, verbosity )
    ...
    !!
    !! DATASETS: ers2-CMCTNC-*-ANT
    !! From the ers2 CMCT Antarctica datasets in netCDF format
    !!
    case ("ers2-cmctnc-wgselev-ant", &
         "ers2-cmctnc-egmmtelev-ant", &
         "ers2-cmctnc-egmtfelev-ant")
       icmct_rec = self%ers2_cmctnc%next_rec( status=nrstatus, verbosity=verb )
       ...
       ! is out of reasonable elevation range.  See ers2_cmctnc%clean for
