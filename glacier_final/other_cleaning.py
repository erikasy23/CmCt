...
def process_link_data_by_region(link_path, selected_regions):
    """Process NetCDF files organized by region folders."""
    summary_frames = []
    
    for region in selected_regions:
        region = str(region).zfill(2)
        region_folder = os.path.join(link_path, f"RGI{region}")
        
        if not os.path.exists(region_folder):
            print(f"Region folder {region_folder} not found. Skipping region {region}")
            continue
            
        # Get all .nc files in this region folder
        nc_files = [f for f in os.listdir(region_folder) if f.endswith(".nc")]
        
        if not nc_files:
            print(f"No .nc files found in {region_folder}")
            continue
        ...
        for fname in nc_files:
            ...
            if len(parts) >= 7:
                ...
                scenario = parts[6]
                key = (model, scenario)
                model_scenario_files.setdefault(key, []).append(os.path.join(region_folder, fname))
        
        # Filter for selected model/scenario
        if (selected_model_text, selected_scenario_text) not in model_scenario_files:
            print(f"No files found for model={selected_model_text}, scenario={selected_scenario_text} in region {region}")
            continue
            
        files = model_scenario_files[(selected_model_text, selected_scenario_text)]
        
        # Process files for this region
        ...
        # Compute changes and save reference glacier data
        elevation_change, mass_change = compute_mass_elevation_changes(combined_ds)
