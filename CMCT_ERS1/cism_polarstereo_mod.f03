...
module cism_polarstereo_mod
  ...
  !! STD_GRN_PARAMS: Parameters for standard CISM PS grid for Greenland.
  ...
  !! feature).
  type( ps_params_def ), parameter :: STD_GRN_PARAMS = ps_params_def( &
       initialized = .false.,  &
       a = 6378.137000_R8b,    &   ! WGS-84, km
       e = 0.08181919104_R8b,  &   ! WGS-84
       northsouth = 1_I4b,     &   ! 1=north
       orientation = 360.0_R8b - 39.0_R8b, &    ! lon 39 deg W
...
contains
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! NAME: cism_ps_init_std_grn
  !!
  !! PURPOSE: Initialize polar_stereographic_mod with the standard CISM
  !! grid for Greenland and the supplied grid spacing, by calling
  ...
  subroutine cism_ps_init_std_grn( spacing, status )

    real(kind=R8b), intent(in) :: spacing
    integer(kind=I4b), intent(out) :: status

    current_params = STD_GRN_PARAMS    ! NB: initialized = .false.
    ...
    if (status /= 0) then
       ...
       print *, 'Error in cism_ps_init_std_grn: ps_SetAndCheckParams status = ', &
            status
    else
       ! We were successful
    ...
    return
  end subroutine cism_ps_init_std_grn
